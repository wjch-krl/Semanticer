	...
	m_syn0norm(NULL), m_dsyn0norm(NULL)
{
	...
	size_t aligment = (size_t)m_vocab_size * m_dim * sizeof(real);
	//
	//  aligment--;
	//  aligment |= aligment >> 1;
	//  aligment |= aligment >> 2;
	//  aligment |= aligment >> 4;
	//  aligment |= aligment >> 8;
	//  aligment |= aligment >> 16;
	//  aligment++;


	m_syn0 = new real[aligment];
	if (m_syn0 == NULL)
	{
