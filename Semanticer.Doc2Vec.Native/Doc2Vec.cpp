#include "headers/Doc2Vec.h"
#include "headers/NN.h"
#include "headers/Vocab.h"
#include "headers/WMD.h"
#include "headers/TrainModelThread.h"
#include "headers/TaggedBrownCorpus.h"
#include <thread>
#include <cstdarg>
                    ...
                    int min_count, int threads)
{
	...
	std::thread** pt = new std::thread*[m_trainModelThreads.size()];
	for (size_t a = 0; a < m_trainModelThreads.size(); a++)
	{
		pt[a] = new std::thread(trainModelThread, (void *)m_trainModelThreads[a]);
	}
	for (size_t a = 0; a < m_trainModelThreads.size(); a++)
	{
		pt[a]->join();
...
real Doc2Vec::similarity(real* src, real* target) const
{
	long long a;
	real dot = 0;
...
real Doc2Vec::distance(real* src, real* target) const
{
	long long a;
	real dis = 0;
...
Vocabulary* Doc2Vec::wvocab()
{
	return m_word_vocab;
}

Vocabulary* Doc2Vec::dvocab()
{
	return m_doc_vocab;
};
