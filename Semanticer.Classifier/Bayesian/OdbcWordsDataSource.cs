...
namespace Semanticer.Classifier.Bayesian
{
    public class OdbcWordsDataSource : ICategorizedWordsDataSource
    {
        ...
        public OdbcWordsDataSource(IDbConnectionManager conMgr, string aTableName, string aWordColumn,
            string aCategoryColumn,
        ...
        public WordProbability GetWordProbability(string category, string word)
        {
            ...
            try
            {
                ...
                IDbCommand command =
                    new OdbcCommand(
                        "SELECT " + MatchCountColumn + ", " + NonMatchCountColumn + " FROM " + TableName + " WHERE " +
                        WordColumn + " = ? AND " + CategoryColumn + " = ?", connection);
                command.Parameters.Add(new OdbcParameter("@Word", OdbcType.VarChar, 255, ParameterDirection.Input, true,
                    0, 0, string.Empty, DataRowVersion.Proposed, word));
                command.Parameters.Add(new OdbcParameter("@Category", OdbcType.VarChar, 20, ParameterDirection.Input,
                    true, 0, 0, string.Empty, DataRowVersion.Proposed, category));
        ...
        private void UpdateWordProbability(string category, string word, bool isMatch)
        {
            ...
            try
            {
                ...
                command =
                    new OdbcCommand(
                        "SELECT * FROM " + TableName + " WHERE " + WordColumn + " = ? AND " + CategoryColumn + " = ?",
                        connection);
                command.Parameters.Add(new OdbcParameter("@Word", OdbcType.VarChar, 255, ParameterDirection.Input, true,
                    0, 0, string.Empty, DataRowVersion.Proposed, word));
                command.Parameters.Add(new OdbcParameter("@Category", OdbcType.VarChar, 20, ParameterDirection.Input,
                ...
                if (!reader.Read()) // word is not in table, so insert the word
                {
                    ...
                    command =
                        new OdbcCommand(
                            "INSERT " + TableName + " (" + WordColumn + ", " + CategoryColumn + ") VALUES (?, ?)",
                            connection);
                    command.Parameters.Add(new OdbcParameter("@Word", OdbcType.VarChar, 255, ParameterDirection.Input,
                        true, 0, 0, string.Empty, DataRowVersion.Proposed, word));
                    command.Parameters.Add(new OdbcParameter("@Category", OdbcType.VarChar, 20, ParameterDirection.Input,
                        true, 0, 0, string.Empty, DataRowVersion.Proposed, category));
                ...
                command =
                    new OdbcCommand(
                        "UPDATE " + TableName + " SET " + fieldName + " = " + fieldName + " + 1 WHERE " + WordColumn +
                        " = ? AND " + CategoryColumn + " = ?", connection);
                command.Parameters.Add(new OdbcParameter("@Word", OdbcType.VarChar, 255, ParameterDirection.Input, true,
                    0, 0, string.Empty, DataRowVersion.Proposed, word));
                command.Parameters.Add(new OdbcParameter("@Category", OdbcType.VarChar, 20, ParameterDirection.Input,
                    true, 0, 0, string.Empty, DataRowVersion.Proposed, category));
        ...
        public void Clear()
        {
            ...
            {
                connection = (OdbcConnection) ConnectionManager.GetConnection();
                OdbcCommand command = new OdbcCommand("DELETE * FROM " + TableName, connection);
                command.ExecuteNonQuery();
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                catch (Exception)
                {
                    command = new OdbcCommand("CREATE TABLE [" + TableName + "] "
                                              ...
                                              + "PRIMARY KEY([" + WordColumn + "], [" + CategoryColumn + "]))",
                        connection);
                    command.ExecuteNonQuery();
                }
            }
