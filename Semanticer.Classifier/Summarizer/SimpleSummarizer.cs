...
namespace Semanticer.Classifier.Summarizer
{
	public class SimpleSummarizer : ISummarizer
	{
		...
		private string[] FindWordsWithFrequency(IDictionary<string,int> wordFrequencies, int frequency)
		{
		    ...
		    {
		        if (frequency == wordFrequencies[word])
		            results.Add(word);
		    }
		    return (string[])results.ToArray(typeof(string));
		}

		protected ArrayList GetMostFrequentWords(int count, Dictionary<string,int> wordFrequencies)
		{
			ArrayList result = new ArrayList();
			int freq = wordFrequencies.Select(entry => entry.Value).Concat(new[] {0}).Max();
		...
		public string Summarize(string input, int numberOfSentences)
		{
			...
			foreach (string word in mostFrequentWords)
			{
				...
				{
					if (workingSentences[i].IndexOf(word, StringComparison.Ordinal) >= 0)
					{
